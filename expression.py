...
class Poly:
    ...
    def __init__(
            self,
            constant: int = 0, x_pow: int = 0, y_pow: int = 0, z_pow: int = 0) -> None:
        self.coeff: defaultdict[int, defaultdict[int, defaultdict[int, int]]] = defaultdict(
            lambda: defaultdict(
                lambda: defaultdict(int)
            ))
        self.set_coeff(constant, x_pow, y_pow, z_pow)

    def is_zero(self) -> bool:
        return len(self.coeff.keys()) == 0

    def all_coeffs(self) -> list[tuple[int, int, int, int]]:
        return [
            (coeff, x_pow, y_pow, z_pow)
    ...
    def get_coeff(self, x_pow: int, y_pow: int, z_pow: int) -> int:
        return self.coeff[x_pow][y_pow][z_pow]

    def set_coeff(self, coeff: int, x_pow: int, y_pow: int, z_pow: int) -> None:
        if coeff == 0:
    ...
    def derivative_x(self) -> Poly:
        ...
        for x_pow in self.coeff.keys():
            ...
            for y_pow, y_val in self.coeff[x_pow].items():
                y_mul = multiplier * (y ** y_pow)
                for z_pow, val in y_val.items():
                    result += y_mul * (z ** z_pow) * Poly(val)
        return result
    ...
    def eval(self, x: int, y: int, z: int) -> int:
        ...
        for x_pow in range(self.highest_x_power()):
            for y_pow, y_val in self.coeff[x_pow].items():
                for z_pow, val in y_val.items():
                    result += x_val * (y ** y_pow) * (z ** z_pow) * val
            x_val *= x
    ...
    def sub_xy(self, xy: Poly) -> Poly:
        ...
        for x_pow in self.coeff.keys():
            for y_pow, y_val in self.coeff[x_pow].items():
                ...
                x_mul = x ** (x_pow - xy_replaced)
                y_mul = y ** (y_pow - xy_replaced)
                new_xy_mul = xy_mul * x_mul * y_mul
                for z_pow, val in y_val.items():
                    z_mul = z ** z_pow
    ...
    def __add__(self, other: Poly) -> Poly:
        ...
        for x_pow, x_val in self.coeff.items():
            for y_pow, y_val in x_val.items():
                for z_pow, val in y_val.items():
                    result.set_coeff(val, x_pow, y_pow, z_pow)
        for x_pow, x_val in other.coeff.items():
            for y_pow, y_val in x_val.items():
                for z_pow, val in y_val.items():
                    result.set_coeff(
                        val+self.get_coeff(x_pow, y_pow, z_pow),
                        x_pow, y_pow, z_pow,
                    )
        return result
    ...
    def __mul__(self, other: Poly) -> Poly:
        ...
        for x_pow1, x_coeff1 in self.coeff.items():
            for y_pow1, y_coeff1 in x_coeff1.items():
                for z_pow1, coeff1 in y_coeff1.items():
                    for x_pow2, x_coeff2 in other.coeff.items():
                        for y_pow2, y_coeff2 in x_coeff2.items():
                            for z_pow2, coeff2 in y_coeff2.items():
                                new_x_pow = x_pow1+x_pow2
                                new_y_pow = y_pow1+y_pow2
                                new_z_pow = z_pow1+z_pow2
                                result.set_coeff(
                                    result.get_coeff(new_x_pow, new_y_pow, new_z_pow) +
                                    coeff1*coeff2,
                                    x_pow1+x_pow2,
                                    y_pow1+y_pow2,
                                    z_pow1+z_pow2,
        ...
        for x_pow, x_val in self.coeff.items():
            for y_pow, y_val in x_val.items():
                for z_pow, coeff in y_val.items():
                    result.set_coeff(coeff % mod, x_pow, y_pow, z_pow)
        return result
    ...
    def _univariate(self, y_pow: int) -> Poly:
        ...
        for x_pow in range(self.highest_x_power()+1):
            result += Poly(self.get_coeff(x_pow, y_pow, 0), x_pow, 0)
        return result
    ...
    def get_x_coefficients_desc(self) -> list[int]:
        ...
        return [self.get_coeff(i, 0, 0) for i in range(high_x, -1, -1)]
...
def remove_x_factor(eqn: Poly) -> Poly:
    ...
    for x in range(eqn.highest_x_power()+1):
        if eqn.get_coeff(x, 0, 0) == 0:
            continue
        if lowest_x is None:
            lowest_x = x
        result.set_coeff(eqn.get_coeff(x, 0, 0), x-lowest_x, 0, 0)
    return result
